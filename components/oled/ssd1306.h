...
public:
	...
	uint8_t draw_string(uint8_t x, uint8_t y, std::string str,
			ssd1306_color_t foreground, ssd1306_color_t background);
	/**
	* @brief   Draw a bitmap on the OLED display
	* @param   x           X position of the top-left corner of the bitmap
	* @param   y           Y position of the top-left corner of the bitmap
	* @param   bitmap      Pointer to the bitmap data (1-bit per pixel)
	* @param   width       Width of the bitmap in pixels
	* @param   height      Height of the bitmap in pixels
	* @param   foreground  Color of the 'on' pixels in the bitmap
	* @param   background  Color of the 'off' pixels in the bitmap
	* @return  void
	*/
