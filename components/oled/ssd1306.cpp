...
bool OLED::init() {
	...
	if (type == SSD1306_128x64) {
		...
		command(address, 0xa0); // SSD1306_SEGREMAP | 1
		command(address, 0xc0); // SSD1306_COMSCANDEC
...
void OLED::draw_bitmap(int x, int y, const uint8_t* bitmap, 
    ...
    for (int row = 0; row < bitmap_height; ++row) {
        ...
        for (int col = 0; col < bitmap_width; ++col) {
            ...
            if (bitmap[row * bitmap_width + col]) {
                draw_pixel(x + col, y + row, WHITE); // 绘制白色像素
            } else {
                draw_pixel(x + col, y + row, BLACK); // 绘制黑色像素
            }
        }
    }
}
